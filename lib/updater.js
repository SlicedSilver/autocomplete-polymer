...
var fileAnalyse = function(content, dictionary, esprima) {
  numberAnalysed += 1;
  var syntaxTree;
  try {
  ...
  if (syntaxTree) {
    for (var x = 0, lenX = syntaxTree.body.length; x < lenX; x++) {
      // Find the Polymer Call Expression in the Syntax Tree
      if (syntaxTree.body[x].type === 'ExpressionStatement' && syntaxTree.body[x].expression.type === 'CallExpression' && syntaxTree.body[x].expression.callee.name === 'Polymer') {
        polymerTree = syntaxTree.body[x];
      }
    }

    if (polymerTree) {
      var polyProps = polymerTree.expression.arguments[0].properties;
        ...
        if (polyProps[y].key.name === 'properties') {
          ...
          for (var z = 0, lenZ = elProperties.length; z < lenZ; z++) {
            if (elProperties[z].key.name) {
              console.log(elProperties[z]);
              elementProperties.push(elProperties[z].key.name);
            }
          }
        }
      }
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (path.extname(filepath).toLowerCase() === '.html') {
      fs.readFile(filepath, 'utf8', (function(_this) {
        return function(error, content) {
          if (error === null) {
            ...
            _this.fileAnalyse(scriptContent, _this.dictionary, _this.esprima);
          }
        };
      ...
      fs.readFile(filepath, 'utf8', (function(_this) {
        return function(error, content) {
          if (error === null) {
            _this.fileAnalyse(content, _this.dictionary, _this.esprima);
          }
        };
      })(fileAnalyseCallbacks));
    }
